...
GEMINI_MODEL_NAME = os.getenv("GOOGLE_AI_MODEL", "gemini-2.5-pro-preview-06-05")
...
try:
    ...
    if not google_api_key:
        ...
        st.error("🔴 Chave da API do Google não encontrada. Por favor, defina `GOOGLE_API_KEY` em um arquivo `.env`.")
...
def delete_file(filename):
    ...
    try:
        if filepath.is_file():
            ...
            st.warning(f"Não foi possível excluir: Arquivo '{filename}' não encontrado.")
            return False
    except Exception as e:
...
def parse_and_execute_ai_commands(ai_response_text):
    ...
    try:
        ...
        if not isinstance(commands, list):
            st.error("A resposta da IA foi JSON válido, mas não uma lista de comandos.")
        ...
        for command_data in commands:
            ...
            if not isinstance(command_data, dict):
                st.warning(f"A IA enviou um formato de comando inválido (não é um dict): {command_data}")
            ...
            if action == "create_update":
                if filename and content is not None:
                    ...
                    if success:
                        ...
                        st.error(f"Falha no comando da IA: Não foi possível salvar '{filename}'.")
                        # Add error details to chat display list
                        executed_commands_list.append({"action": "chat", "content": f"Error: Failed saving {filename}"})
                else:
                    st.warning("Comando 'create_update' da IA sem nome de arquivo ou conteúdo.")
                ...
                if filename:
                    ...
                    if not success:
                         st.error(f"Falha no comando da IA: Não foi possível excluir '{filename}'.")
        ...
        st.error(f"A resposta da IA não foi JSON válido.\nResposta bruta:\n```\n{cleaned_text}\n```")
...
def ask_gemini_ai(chat_history):
    ...
    try:
        ...
        if "API key not valid" in str(e):
            ...
            error_content = "Erro da IA: Cota ou Limite de Taxa da API Excedido."
        ...
        try:
            ...
            if process_to_stop.poll() is None:
                ...
                try:
                    ...
                    st.toast(f"Processo de visualização {pid} parado.", icon="⏹️")
                except subprocess.TimeoutExpired:
                    # If it didn't stop, force kill it
                    st.warning(f"Processo de visualização {pid} não parou normalmente, matando...")
                    if process_to_stop.poll() is None: # Check again before kill
                        process_to_stop.kill()
                        process_to_stop.wait(timeout=1) # Brief wait for kill
                        st.toast(f"Processo de visualização {pid} morto.", icon="💀")
            else:
                # Process was already finished
                st.warning(f"Processo de visualização {pid} já havia parado.")
        except ProcessLookupError:
            st.warning(f"Processo de visualização {pid} não encontrado (já foi embora?).")
        except Exception as e:
            st.error(f"Erro ao tentar parar processo de visualização {pid}: {e}")
...
def start_preview(python_filename):
    ...
    if not filepath.is_file() or filepath.suffix != '.py':
        st.error(f"Não é possível visualizar: '{python_filename}' não é um arquivo Python válido.")
        return False
    ...
    with st.spinner(f"Iniciando visualização para `{python_filename}`..."):
        try:
            ...
            if preview_proc.poll() is None:
                ...
                st.success(f"Visualização iniciada: {st.session_state.preview_url}")
                st.toast(f"Visualização em execução para {python_filename}", icon="🚀")
                return True
            else:
                # Failure: Process ended quickly, likely an error
                st.error(f"Falha ao iniciar visualização para `{python_filename}`.")
                ...
                try:
                    ...
                    if stderr_output:
                        ...
                        st.code(stderr_output, language=None)
                         ...
                         if stdout_output:
                              st.error("Saída da Visualização (pode conter erros):")
                              st.code(stdout_output, language=None)
                except Exception as read_e:
                    st.error(f"Não foi possível ler saída do processo de visualização falhado: {read_e}")
...
with st.sidebar:
                ...
                with st.chat_message(role, avatar=avatar):
                    if role == "assistant" and isinstance(content, list):
                        ...
                        for command in content:
                            ...
                            if action == "create_update":
                                file_actions_summary += f"📝 **Salvo:** `{filename}`\n"
                                if cmd_content:
                                    code_snippets.append({"filename": filename, "content": cmd_content})
                            elif action == "delete":
                                file_actions_summary += f"🗑️ **Excluído:** `{filename}`\n"
                            elif action == "chat":
                                chat_responses.append(str(cmd_content or "..."))
                            else:
                                file_actions_summary += f"⚠️ **Ação Desconhecida:** `{action}`\n"
                        ...
                        for snippet in code_snippets:
                            with st.expander(f"Ver Código para `{snippet['filename']}`", expanded=False):
    ...
    # --- Footer ---
    st.write("Desenvolvido com ❤️ por:\n[Matheus Fabião](https://github.com/matheusfabiao)", unsafe_allow_html=True)
...
if selected_tab == "Workspace":
    ...
    with editor_col:
        ...
        if selected_filename:
            ...
            editor_buttons = [
                sac.ButtonsItem(label="💾 Salvar Alterações", icon="save", disabled=not has_unsaved_changes),
                sac.ButtonsItem(label="🗑️ Excluir Arquivo", icon="trash", color="red"),
            ]
            ...
            if clicked_editor_button == "💾 Salvar Alterações":
                if save_file(selected_filename, editor_current_text):
                    ...
                    st.error(f"Erro: Não foi possível salvar '{selected_filename}'.")
            ...
            st.info("Selecione um arquivo Python da lista à esquerda para visualizar ou editar.")
            st_ace(value="# Selecione um arquivo...", language="python", readonly=True, key="ace_placeholder")

# --- Live Preview Tab ---
elif selected_tab == "Visualização ao Vivo":
    st.header("▶️ Visualização ao Vivo")
    st.divider()
    st.warning("⚠️ Executar código gerado pela IA pode ter consequências não intencionais. Revise o código primeiro!")
    ...
    if not selected_file_for_preview:
        st.info("Selecione um arquivo na aba 'Workspace' para habilitar os controles de visualização.")
        # Allow stopping a preview even if no file is selected
        if is_preview_running:
            st.warning(f"Visualização em execução para: `{file_being_previewed}`")
            if st.button(f"⏹️ Parar Visualização ({file_being_previewed})", key="stop_other_preview"):
                stop_preview() # Will stop and rerun
    else:
        # Controls for the file selected in the Workspace
        st.write(f"Arquivo selecionado para visualização: `{selected_file_for_preview}`")
        is_python = selected_file_for_preview.endswith(".py")

        if not is_python:
            st.error("Não é possível visualizar: Arquivo selecionado não é um arquivo Python (.py).")
        else:
            ...
            with run_col:
                ...
                if st.button("🚀 Executar Visualização", disabled=run_disabled, type="primary", use_container_width=True):
            ...
            with stop_col:
                ...
                if st.button("⏹️ Parar Visualização", disabled=stop_disabled, use_container_width=True):
                    stop_preview() # Will stop and rerun
    ...
    if is_preview_running:
        ...
        if file_being_previewed == selected_file_for_preview:
            ...
            if live_process and live_process.poll() is None:
                ...
                st.warning(f"Visualização para `{file_being_previewed}` parou inesperadamente.")
                # Attempt to show error output if available
                if live_process:
                     try:
                         stderr = live_process.stderr.read()
                         if stderr:
                              with st.expander("Mostrar saída de erro do processo parado"): st.code(stderr)
            ...
            st.warning(f"Visualização em execução para `{file_being_previewed}`. Selecione esse arquivo no Workspace para vê-lo aqui, ou pare-o usando os controles acima.")
    else:
        # No preview is currently running
        st.info("Clique 'Executar Visualização' em um arquivo Python selecionado para vê-lo aqui.")
